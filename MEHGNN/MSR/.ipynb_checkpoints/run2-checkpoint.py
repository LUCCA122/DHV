...
from utils.data import load_MSR_data
from utils.tools import index_generator, parse_minibatch_LastFM
from model.MEHGNN_lp2 import MEHGNN_lp2
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, top_k_accuracy_score

from model.Diff import KGHandler, Denoise,GaussianDiffusion
from utils.TimeLogger import log ,contrastLoss
...
def run_net_LastFM(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
                     num_epochs, patience, batch_size, neighbor_samples, repeat, save_postfix):
    adjlists_ua, edge_metapath_indices_list_ua, adjM, type_mask, train_val_test_pos_user_artist, train_val_test_neg_user_artist = load_MSR_data()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  
    torch.cuda.synchronize()

    # 参数设置  # 嵌入向量维度
    n_hops = 2   # 信息传播的轮数
    kg_file = 'new_kg.txt'  # 知识图谱文件路径
    ...
    # 获取边索引和关系类型
    n_hops = 2
    entity_n =handler.entity_n
    relation_n = handler.relation_n
    ...
    for _ in range(repeat):
        ...
        num_pvs = (type_mask == 1).sum()
        num_cves = (type_mask == 0).sum()
        Mat=adjM[num_cves:,:num_cves]

        handler.LoadData(Mat,batch_size=batch_size)

        net = MEHGNN_lp2(
            [1, 1], 2, etypes_lists, in_dims, hidden_dim, hidden_dim, num_heads, attn_vec_dim,n_hops,entity_n,relation_n, rnn_type, dropout_rate)
        net.to(device)
        optimizer = torch.optim.Adam(net.parameters(), lr=lr, weight_decay=weight_decay)

        diffusion_model = GaussianDiffusion(noise_scale = 0.1, noise_min=0.01, noise_max = 0.1, steps = 5).cuda()

        out_dim = eval('[1000]') + [entity_n]
        in_dim = out_dim[::-1]
        denoise_model = Denoise(in_dim, out_dim, emb_size=20, norm=True).cuda()
        denoise_opt = torch.optim.Adam(denoise_model.parameters(), lr=lr, weight_decay=0)
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in enumerate(diffusionLoader):
                ...
                ui_matrix = handler.ui_matrix
                iEmbeds = net.getEntityEmbeds().detach()
                uEmbeds = net.getUserEmbeds().detach()

                denoise_opt.zero_grad()

                diff_loss, ukgc_loss = diffusion_model.training_losses(denoise_model, batch_item, ui_matrix, uEmbeds, iEmbeds, batch_index)
                loss = diff_loss.mean() * 0.5 + ukgc_loss.mean() * 0.5
                ...
                log('Diffusion Step %d/%d' % (i, diffusionLoader.dataset.__len__() // batch_size), save=False, oneline=True)
            ...
            with torch.no_grad():
                ...
                for _, batch in enumerate(diffusionLoader):
                    batch_item, batch_index = batch
                    batch_item, batch_index = batch_item.cuda(), batch_index.cuda()
                    denoised_batch = diffusion_model.p_sample(denoise_model, batch_item, 0)
                    top_item, indices_ = torch.topk(denoised_batch, k=10)
            ...
            for iteration in range(train_pos_idx_generator.num_iterations()):
                ...
                [pos_embedding_user, pos_embedding_artist], _ = net(
                    (train_pos_g_lists, features_list, type_mask, train_pos_indices_lists, train_pos_idx_batch_mapped_lists),denoisedKG)
                [kg_pos_embedding_user, kg_pos_embedding_artist], _ = net(
                    (train_pos_g_lists, features_list, type_mask, train_pos_indices_lists, train_pos_idx_batch_mapped_lists),kg)    
                [neg_embedding_user, neg_embedding_artist], _ = net(
                    (train_neg_g_lists, features_list, type_mask, train_neg_indices_lists, train_neg_idx_batch_mapped_lists),denoisedKG)
                [kg_neg_embedding_user, kg_neg_embedding_artist], _ = net(
                    (train_pos_g_lists, features_list, type_mask, train_pos_indices_lists, train_pos_idx_batch_mapped_lists),kg)    
                
                pos_closs = contrastLoss(pos_embedding_user,kg_pos_embedding_user,torch.arange(pos_embedding_user.size(0)),0.1)+contrastLoss(pos_embedding_artist,kg_pos_embedding_artist,torch.arange(pos_embedding_artist.size(0)),0.1)
                neg_closs = contrastLoss(neg_embedding_user,kg_neg_embedding_user,torch.arange(neg_embedding_user.size(0)),0.1)+contrastLoss(neg_embedding_artist,kg_neg_embedding_artist,torch.arange(neg_embedding_artist.size(0)),0.1)
                '''
                
                [pos_embedding_user, pos_embedding_artist], _ = net(
                    (train_pos_g_lists, features_list, type_mask, train_pos_indices_lists, train_pos_idx_batch_mapped_lists),kg,denoisedKG)
                [neg_embedding_user, neg_embedding_artist], _ = net(
                    (train_neg_g_lists, features_list, type_mask, train_neg_indices_lists, train_neg_idx_batch_mapped_lists),kg,denoisedKG)
                #计算损失
                ...
                #closs = (pos_closs+neg_closs)*(1e-2)
                #train_loss = closs + train_loss
            ...
            with torch.no_grad():
                for iteration in range(val_idx_generator.num_iterations()):
                    ...
                    [pos_embedding_user, pos_embedding_artist], _ = net(
                        (val_pos_g_lists, features_list, type_mask, val_pos_indices_lists, val_pos_idx_batch_mapped_lists),kg,denoisedKG)
                    [neg_embedding_user, neg_embedding_artist], _ = net(
                        (val_neg_g_lists, features_list, type_mask, val_neg_indices_lists, val_neg_idx_batch_mapped_lists),kg,denoisedKG)
                    pos_embedding_user = pos_embedding_user.view(-1, 1, pos_embedding_user.shape[1])
                    pos_embedding_artist = pos_embedding_artist.view(-1, pos_embedding_artist.shape[1], 1)
                    neg_embedding_user = neg_embedding_user.view(-1, 1, neg_embedding_user.shape[1])
                    neg_embedding_artist = neg_embedding_artist.view(-1, neg_embedding_artist.shape[1], 1)
        ...
        with torch.no_grad():
            for iteration in range(test_idx_generator.num_iterations()):
                ...
                [pos_embedding_user, pos_embedding_artist], _ = net(
                    (test_pos_g_lists, features_list, type_mask, test_pos_indices_lists, test_pos_idx_batch_mapped_lists),kg,generatedKG)
                [neg_embedding_user, neg_embedding_artist], _ = net(
                    (test_neg_g_lists, features_list, type_mask, test_neg_indices_lists, test_neg_idx_batch_mapped_lists),kg,generatedKG)
                pos_embedding_user = pos_embedding_user.view(-1, 1, pos_embedding_user.shape[1])
                pos_embedding_artist = pos_embedding_artist.view(-1, pos_embedding_artist.shape[1], 1)
                neg_embedding_user = neg_embedding_user.view(-1, 1, neg_embedding_user.shape[1])
                neg_embedding_artist = neg_embedding_artist.view(-1, neg_embedding_artist.shape[1], 1)
...
if __name__ == '__main__':
    ...
    ap.add_argument('--save-postfix', default='LastFM', help='Postfix for the saved net and result. Default is LastFM.')

    args = ap.parse_args()
    run_net_LastFM(args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type, args.epoch,
