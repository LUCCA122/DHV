# -*- coding: utf-8 -*-
"""
Created on Tue Jul  9 10:36:16 2024
...
插入t-SNE代码以及评估代码的步骤
1.在训练循环完成后，切换到评估模式
2.使用net.eval()切换到评估模式。
3.生成测试数据的特征向量
4.使用t-SNE将高维特征向量映射到二维平面
5.可视化t-SNE的结果
            ...
            if epoch % 5 == 0:
                ...
                with torch.no_grad():
                    test_pos_g_lists, test_pos_indices_lists, test_pos_idx_batch_mapped_lists = parse_minibatch_LastFM(
                        adjlists_ua, edge_metapath_indices_list_ua, test_pos_user_artist.tolist(), device, neighbor_samples, use_masks, num_user)
                    test_neg_g_lists, test_neg_indices_lists, test_neg_idx_batch_mapped_lists = parse_minibatch_LastFM(
                        adjlists_ua, edge_metapath_indices_list_ua, test_neg_user_artist.tolist(), device, neighbor_samples, no_masks, num_user)
        
                    [test_pos_embedding_user, test_pos_embedding_artist], _ = net(
                        (test_pos_g_lists, features_list, type_mask, test_pos_indices_lists, test_pos_idx_batch_mapped_lists))
                    [test_neg_embedding_user, test_neg_embedding_artist], _ = net(
                        (test_neg_g_lists, features_list, type_mask, test_neg_indices_lists, test_neg_idx_batch_mapped_lists))
    
                # Concatenate positive and negative samples
                test_embeddings = torch.cat([test_pos_embedding_user, test_neg_embedding_user], dim=0).cpu().numpy()
                test_artists_embeddings = torch.cat([test_pos_embedding_artist, test_neg_embedding_artist], dim=0).cpu().numpy()

                # Apply t-SNE
                tsne = TSNE(n_components=2, random_state=42)
                user_embeddings_2d = tsne.fit_transform(test_embeddings)
                artist_embeddings_2d = tsne.fit_transform(test_artists_embeddings)
    
                # Plot t-SNE result
                plt.figure(figsize=(10, 8))
                plt.scatter(user_embeddings_2d[:len(test_pos_user_artist), 0], user_embeddings_2d[:len(test_pos_user_artist), 1], c='red', label='Positive Users', alpha=0.5)
                plt.scatter(user_embeddings_2d[len(test_pos_user_artist):, 0], user_embeddings_2d[len(test_pos_user_artist):, 1], c='blue', label='Negative Users', alpha=0.5)
                plt.scatter(artist_embeddings_2d[:len(test_pos_user_artist), 0], artist_embeddings_2d[:len(test_pos_user_artist), 1], c='orange', label='Positive Artists', alpha=0.5)
                plt.scatter(artist_embeddings_2d[len(test_pos_user_artist):, 0], artist_embeddings_2d[len(test_pos_user_artist):, 1], c='cyan', label='Negative Artists', alpha=0.5)
                plt.legend()
               # plt.title('t-SNE Visualization of User and Artist Embeddings')
                plt.title(f't-SNE Visualization of User and Artist Embeddings in epoch {epoch}')
                plt.show()
